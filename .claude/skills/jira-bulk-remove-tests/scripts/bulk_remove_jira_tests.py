#!/usr/bin/env python3
"""
Remove multiple Jira test issues in bulk with batching support.

Usage: python bulk_remove_jira_tests.py <TEST-KEY-1> <TEST-KEY-2> ...
Example: python bulk_remove_jira_tests.py CXREC-107916 CXREC-107917 CXREC-107918

Workflow per test:
  - If "Approved" → Open → Removed
  - Otherwise → Removed
...
def remove_test(test_key):
    ...
    if current_status and current_status.lower() == "approved":
        resp = fetch_transitions(test_key)
        transitions = resp.get("transitions", [])
        tid = find_transition_to_status(transitions, "open") or find_transition_to_status(transitions, "re-open")
        if not tid:
            raise Exception(f'No "Open" transition found for approved test {test_key}')
        execute_transition(test_key, tid)

    resp = fetch_transitions(test_key)
    tid = find_transition_to_status(resp.get("transitions", []), "removed")
    if not tid:
        raise Exception(f'No "Removed" transition found for {test_key}')
    execute_transition(test_key, tid)
...
def main():
    ...
    if not test_keys:
        print("Usage: python bulk_remove_jira_tests.py <TEST-KEY-1> <TEST-KEY-2> ...", file=sys.stderr)
        sys.exit(1)
