#!/usr/bin/env python3
"""
Shared Xray Cloud GraphQL client for the test-cases-optimizer skill scripts.

Credentials are read from the machine's environment variables; they are never
hardcoded.
...
Optional Environment Variables:
  JIRA_BASE_URL       - Jira base URL for building issue links (default below)

Get credentials from: https://xray.cloud.getxray.app/api/settings
"""
...
DEFAULT_JIRA_BASE_URL = "https://nice-ce-cxone-prod.atlassian.net"
FETCH_TIMEOUT_S = 30
...
def get_jira_base_url():
    return (os.environ.get("JIRA_BASE_URL") or DEFAULT_JIRA_BASE_URL).rstrip("/")
...
def get_xray_credentials():
    ...
    if not client_id or not client_secret:
        ...
        print("\nSet them as environment variables:", file=sys.stderr)
        print('  Windows (persistent): setx XRAY_CLIENT_ID "your_client_id"', file=sys.stderr)
        print('  Windows (session):    $env:XRAY_CLIENT_ID = "your_client_id"', file=sys.stderr)
